...
import com.fenrir.masterdetail.model.statistics.*;
...
public class StatisticsRepository {
    ...
    public List<CovidDayStatistics> findCovidDayByDayStatistics() {
        Query query = entityManager.createQuery(
                "SELECT new com.fenrir.masterdetail.model.statistics.CovidDayStatistics(cr.date, SUM(cr.newCases), SUM(cr.newDeaths)) " +
    ...
    public List<CovidMonthStatistics> findCovidMonthByMonthStatistics() {
        Query query = entityManager.createQuery(
                "SELECT new com.fenrir.masterdetail.model.statistics.CovidMonthStatistics(YEAR(cr.date) AS y, MONTH(cr.date) AS m, SUM(cr.newCases), SUM(cr.newDeaths)) " +
                        "FROM CovidReport AS cr " +
    ...
    public List<CovidYearStatistics> findCovidYearByYearStatistics() {
        Query query = entityManager.createQuery(
                "SELECT new com.fenrir.masterdetail.model.statistics.CovidYearStatistics(YEAR(cr.date) AS y, SUM(cr.newCases), SUM(cr.newDeaths)) " +
                        "FROM CovidReport AS cr " +
    ...
    public List<MovieYearStatistics> findMovieCountYearByYear() {
        Query query = entityManager.createQuery(
                "SELECT new com.fenrir.masterdetail.model.statistics.MovieYearStatistics(m.startYear, COUNT(m.id)) " +
                        "FROM Movie AS m " +
    ...
    public List<MovieYearStatistics> findMoviePopularityYearByYear() {
        Query query = entityManager.createQuery(
                "SELECT new com.fenrir.masterdetail.model.statistics.MovieYearStatistics(m.startYear, SUM(m.numVotes)) " +
                        "FROM Movie AS m " +
                        "GROUP BY m.startYear " +
                        "ORDER BY m.startYear"
        );
    ...
    public List<MovieAverageRateYearStatistics> findMovieAverageRatingYearByYear() {
        Query query = entityManager.createQuery(
                "SELECT new com.fenrir.masterdetail.model.statistics.MovieAverageRateYearStatistics(m.startYear, AVG(m.averageRating)) " +
                        "FROM Movie AS m " +
                        "GROUP BY m.startYear " +
                        "ORDER BY m.startYear"
        );
